...
tprime = 10540800; %four months
tinterval = 25*60;%25 second interval%60*60*6; %time interval in seconds
...
%issue with initial conditions? MICE might be more accurate; might need to
%input initcondits into simulink

x0 = state_e(1,1)-500;
y0 = state_e(1,2);
z0 = state_e(1,3);
vx0 = state_e(1,4);
vy0 = state_e(1,5)-7.6118;
vz0 = state_e(1,6);
m0 = 9700;
...
%if tinterval == 1
   for i=1:length(t)
      if i>=0 && i<= 10%1500 %25 minute burn
          THRUST(i) = MaxMag*0; %assumed instantaneous reactor
      elseif i>=86400 && i<=87900
          THRUST(i) = MaxMag*0;
...
posit = plot3(state_e(:,1),state_e(:,2),state_e(:,3),'-ob',state_m(:,1),state_m(:,2),state_m(:,3),'-or',state_j(:,1),state_j(:,2),state_j(:,3),'-om',state_p(:,1),state_p(:,2),state_p(:,3),'-xk',state_l(:,1),state_l(:,2),state_l(:,3),'-xc',state_d(:,1),state_d(:,2),state_d(:,3),'-xy',x,y,z,'og',0,0,0,'oy');
...
ma = length(state_m(:,1));
jo = length(x);
positmars = plot3(0,0,0,'or',state_p(:,1)-state_m(:,1),state_p(:,2)-state_m(:,2),state_p(:,3)-state_m(:,3),'-xk',state_d(:,1)-state_m(:,1),state_d(:,2)-state_m(:,2),state_d(:,3)-state_m(:,3),'-xy',x(jo)-state_m(ma,1),y(jo)-state_m(ma,2),z(jo)-state_m(ma,3),'og');
